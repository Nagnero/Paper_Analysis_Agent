# Korean Paper Agent Console (KPAC) · v0.5.0
...
| **에이전트** | orchestrator → analyst → verifier → writer (+coreInsight, +근거탐색) | writeOrchestrator → plan → body/figure → review → citation → compile (+근거탐색) |
...
- **ZIP 드롭 → 자동 컴파일** — 채팅 없이 바로 컴파일된 프로젝트가 라이브러리에 생성
- **디렉터리 트리** — `figures/`, `sections/` 등 하위 폴더가 접이식 트리로 표시
- **인앱 에디터 (Monaco)** — `.tex/.bib/.cls/.sty` 등 텍스트 파일 편집·저장(자동 저장), LaTeX 문법 강조
- **이미지 열람 & 업로드** — `png/jpg/jpeg/gif/webp/bmp/svg` 를 트리에서 클릭하면 미리보기. **헤더 "⬆ 이미지" 버튼 또는 드래그&드롭**으로 그림 파일을 프로젝트에 추가(20MB 상한)
- **SyncTeX 역방향 점프** — PDF 의 한 지점을 더블클릭하면 대응하는 `.tex` 줄로 이동 + 하이라이트
- **컴파일 상태 배지 + 로그** — 성공/경고/실패 표시, 로그 패널 토글
- **프로젝트 ZIP 다운로드** — 편집한 프로젝트를 통째로 내려받기(중간 산출물 제외)

### AI 작성팀 (STORM식 멀티에이전트)

채팅으로 수정을 지시하면, **오케스트레이터가 지시를 분류**해 적절한 모듈로 보내고, 본문·그림/표 작업은 **계획 → 작성 → 검토** 3단계 멀티에이전트로 처리합니다. 수정 후에는 **컴파일 게이트**가 항상 실행되어 오류가 있으면 자동으로 고칩니다. 진행 단계는 채팅에 **실시간(SSE)** 으로 표시됩니다.

```
[작성 트리거] 채팅 지시 (예: "introduction 을 더 간결하게")
─────────────────────────────────────────────────────────────
  ① writeOrchestrator   지시 분류 → writing / figure / citation / evidence
        │
   ┌────┴───────────────────────────┐
   본문·그림/표 (멀티에이전트)          인용·근거탐색 (싱글)
   ② writePlan    계획 수립            · writeCitation : 빈 \cite{} 를 .bib 근거로 채움
   ③ writeBody /                       · evidence      : 읽기 전용 — 문서에서 근거만 찾아 답
      writeFigure  작성
   ④ writeReview  검토·다듬기
        │
        ▼
  ⑤ 컴파일 게이트 — 실패 시 writeCompile 이 로그 보고 자동 수정 (최대 2회)
        │
        ▼
  수정된 .tex 저장 + 재컴파일된 PDF + 단계별 SSE 스트림
```

- **표(table) 1급 지원** — 그림·표 모듈이 tikz/그림뿐 아니라 `booktabs`·`tabularx`·`multirow`/`multicolumn` 표를 동등하게 생성·수정
- **논문 작성 스타일 규칙 6종** (작성·검토 양쪽에서 강제):
  ...
  5. 뒷받침 못 하는 내용엔 빈 `\cite{}` 만 표시(키는 인용 에이전트가 채움)
  6. 볼드·이탤릭 없이 줄글로 서술
